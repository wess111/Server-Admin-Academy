(() => {
  ...
  function renderInventory(){
    const type=$("#componentCategory").value||slotTypes[0]; const items=catalog.filter(c=>c.type===type);
    $("#inventoryCount").textContent=`${items.length} option${items.length===1?"":"s"}`;
    $("#inventoryList").innerHTML=items.map(c=>{
      const isInstalled=c.type==="ram"?installedRams().some(x=>x.component.id===c.id):state.installed[c.type]===c.id;
      const selected=state.selectedId===c.id;
      return `<button class="simPart ${selected?"isSelected":""} ${isInstalled?"isInstalled":""}" data-component="${esc(c.id)}" type="button" ${!state.active?"disabled":""} aria-pressed="${selected?"true":"false"}"><span class="simPartIcon" aria-hidden="true">${esc(categoryIcons[c.type]||"PC")}</span><span><strong>${esc(c.name)}</strong><small>${esc(c.detail)}</small></span><span class="simPartPrice"><span class="srOnly">Scenario price </span>${money(c.price)}</span></button>`
    }).join("");
    $$(".simPart").forEach(btn=>btn.addEventListener("click",()=>{
      state.selectedId=btn.dataset.component;state.focusedInstalled=null;
      const c=comp(state.selectedId);
      $("#selectedPart").textContent=c.name;
      $("#selectedPartHelp").textContent=`${c.detail} • Scenario price ${money(c.price)}. Choose a compatible installation target.`;
      const research=$("#researchPartLink");
      if(c.researchUrl){research.href=c.researchUrl;research.hidden=false;research.setAttribute("aria-label",`Open manufacturer specifications for ${c.name} in a new tab`)}
      else{research.hidden=true;research.removeAttribute("href")}
      setConsole(`${c.name} selected. Choose the correct workbench location.`,"SELECT");
      renderInstalledActions();renderInventory();
    }));
  }

  function renderInstalledActions(){
    const box=$("#installedActions"), remove=$("#removeInstalledBtn");
    if(!box||!state.focusedInstalled){if(box)box.hidden=true;return}
    const ref=state.focusedInstalled;
    const c=ref.ramSlot?comp(state.ramSlots[ref.ramSlot]):installed(ref.type);
    if(!c){box.hidden=true;state.focusedInstalled=null;return}
    box.hidden=false;
    $("#installedActionName").textContent=ref.ramSlot?`${c.name} — DIMM ${ref.ramSlot}`:c.name;
    $("#installedActionHelp").textContent=ref.type==="motherboard"
      ?"Removing the motherboard also removes dependent CPU, cooling, memory, storage, and graphics components."
      :"Removal is explicit so an accidental click cannot undo your work.";
    remove.setAttribute("aria-label",`Remove ${c.name}${ref.ramSlot?` from DIMM ${ref.ramSlot}`:""}`);
  }

  function focusInstalled(ref){
    state.selectedId=null;state.focusedInstalled=ref;
    const c=ref.ramSlot?comp(state.ramSlots[ref.ramSlot]):installed(ref.type);
    if(c)setConsole(`${c.name} selected for inspection. Use the explicit Remove component button if you intend to uninstall it.`,"INSPECT");
    renderInstalledActions();renderInventory();
  }

  function removeFocusedInstalled(){
    const ref=state.focusedInstalled;if(!ref)return;
    const c=ref.ramSlot?comp(state.ramSlots[ref.ramSlot]):installed(ref.type);if(!c)return;
    if(ref.ramSlot){state.ramSlots[ref.ramSlot]=null}
    else{
      const target=ref.type;
      if(target==="motherboard"&&!window.confirm("Remove the motherboard and all components installed on it?"))return;
      state.installed[target]=null;
      if(target==="motherboard"){["cpu","paste","cooler","ram","storage","gpu"].forEach(t=>state.installed[t]=null);Object.keys(state.ramSlots).forEach(k=>state.ramSlots[k]=null);state.cables={atx:false,eps:false,gpu:false,cpuFan:false}}
      if(target==="cpu"){state.installed.paste=null;state.installed.cooler=null;state.cables.eps=false;state.cables.cpuFan=false}
      if(target==="cooler")state.cables.cpuFan=false;
      if(target==="gpu")state.cables.gpu=false;
      if(target==="psu")state.cables={...state.cables,atx:false,eps:false,gpu:false};
    }
    state.focusedInstalled=null;state.powered=false;state.post=null;state.validated=false;state.changes++;
    setConsole(`${c.name} removed from the build.`,"REMOVE");render();
  }
  ...
  function updateSlot(type){
    ...
    const slot=$(`[data-slot="${type}"]`); if(slot){slot.classList.toggle("isInstalled",!!c);slot.classList.remove("isIncompatible");slot.setAttribute("aria-label",c?`${categoryLabels[type]} installation area. ${c.name} installed. Press Enter to inspect.`:`${categoryLabels[type]} installation area. Empty.`)}
  }
  ...
  function render(){
    ...
    const mb=installed("motherboard"), chipset=$("#board-chipset");if(chipset)chipset.textContent=mb?.chipset||"CHIPSET";
    renderRamSlots();renderInstalledActions();renderInventory();
  }
  ...
  function installSelected(target){
    ...
    if(current){focusInstalled({type:target});return}
    ...
    state.installed[target]=c.id;state.selectedId=null;state.focusedInstalled=null;state.powered=false;state.post=null;state.validated=false;state.changes++;
    setConsole(`${c.name} installed. Installed count ${installedCount()}; current scenario cost ${money(totalCost())}.`,"INSTALL");render();
  }

  function renderRamSlots(){
    Object.keys(state.ramSlots).forEach(slot=>{
      const c=comp(state.ramSlots[slot]), el=document.querySelector(`[data-ram-slot="${slot}"]`), label=document.getElementById(`state-ram-${slot}`);
      if(el){el.classList.toggle("isInstalled",!!c);el.setAttribute("aria-label",c?`DIMM ${slot}. ${c.name} installed. Press Enter to inspect.`:`DIMM ${slot}. Empty${["A2","B2"].includes(slot)?". Preferred population slot":""}.`)}
  ...
  function installRam(slot){
    if(!state.active){setConsole("Start an attempt before using the workbench.","WAIT");return}
    const current=comp(state.ramSlots[slot]);if(current){focusInstalled({type:"ram",ramSlot:slot});return}
    if(!state.selectedId){setConsole(`Choose a memory module from the component catalog, then select DIMM ${slot}.`,"WAIT");return}
    const c=comp(state.selectedId);if(c.type!=="ram"){setConsole(`${c.name} cannot be installed in DIMM ${slot}. Select a memory module.`,"INVALID");return}
    if(!installed("motherboard")){setConsole("Install a motherboard before adding memory.","ORDER");return}
    state.ramSlots[slot]=c.id;state.selectedId=null;state.focusedInstalled=null;state.powered=false;state.post=null;state.validated=false;state.changes++;
    setConsole(`${c.name} installed in DIMM ${slot}. Total memory: ${totalMemoryGB()} GB • ${memoryMode()} • scenario cost ${money(totalCost())}.`,"INSTALL");render();
  }
  ...
  $("#componentCategory").addEventListener("change",()=>{state.selectedId=null;state.focusedInstalled=null;$("#selectedPart").textContent="None";$("#selectedPartHelp").textContent="Choose a component, then choose its installation area.";$("#researchPartLink").hidden=true;renderInstalledActions();renderInventory()});
  ...
  $("#removeInstalledBtn")?.addEventListener("click",removeFocusedInstalled);
