(() => {
  ...
  function totalCost(){return slotTypes.reduce((sum,t)=>sum+(installed(t)?.price||0),0)}
  function installedCount(){return slotTypes.filter(t=>state.installed[t]).length}
  function resetRunState(){state.selectedId=null;slotTypes.forEach(t=>state.installed[t]=null);state.cables={atx:false,eps:false,gpu:false,cpuFan:false};state.powered=false;state.post=null;state.validated=false;state.validationSnapshot=null;state.diagnostics=0;state.changes=0}

  function compatibilityIssues(){
    const issues=[]; const mb=installed("motherboard"), cpu=installed("cpu"), cooler=installed("cooler"), ram=installed("ram"), gpu=installed("gpu"), psu=installed("psu");
    if(mb&&cpu&&mb.socket!==cpu.socket) issues.push(`CPU socket ${cpu.socket} does not match motherboard socket ${mb.socket}`);
    if(mb&&ram&&mb.memoryType!==ram.memoryType) issues.push(`${ram.memoryType} memory is incompatible with a ${mb.memoryType} motherboard`);
  ...
  function assemblyIssues(){
    const issues=[];
    ["motherboard","cpu","paste","cooler","ram","storage","psu","casefan"].forEach(t=>{if(!state.installed[t])issues.push(`${categoryLabels[t]} not installed`)});
  ...
  function postBlockingIssues(){
    const issues=[];const mb=installed("motherboard"),cpu=installed("cpu"),ram=installed("ram"),psu=installed("psu"),gpu=installed("gpu"),cooler=installed("cooler");
    if(!mb)issues.push("motherboard missing"); if(!cpu)issues.push("CPU missing"); if(!ram)issues.push("memory missing"); if(!psu)issues.push("power supply missing");
  ...
  function requirementResults(){
    if(!scenario)return[];
    const r=scenario.requirements, mb=installed("motherboard"),cpu=installed("cpu"),ram=installed("ram"),storage=installed("storage"),gpu=installed("gpu"),psu=installed("psu");
    const videoOutputs=gpu?.displayOutputs || ((cpu?.integratedGraphics&&mb)?mb.displayOutputs:0);
    const compat=compatibilityIssues();
    const assemblySafe=!!mb&&!!cpu&&!!state.installed.paste&&!!installed("cooler")&&!!ram&&!!storage&&!!psu&&!!installed("casefan")&&state.cables.atx&&state.cables.eps&&state.cables.cpuFan&&(!gpu?.requiresPower||state.cables.gpu)&&(!psu||psu.wattage>=requiredWattage());
    return [
      {key:"memory",label:`Memory: at least ${r.memoryGB} GB`,detail:ram?`${ram.capacityGB} GB ${ram.memoryType} installed`:"No memory installed",pass:!!ram&&ram.capacityGB>=r.memoryGB},
  ...
  function generateReport(){
    ...
    const name=$("#technicianName").value.trim(), date=$("#activityDate").value, results=requirementResults(), components=slotTypes.map(t=>installed(t)).filter(Boolean), id=`SAA-SIM-${Date.now()}`;
  ...
  $("#inspectBtn").addEventListener("click",()=>{const names=slotTypes.map(t=>installed(t)?.name).filter(Boolean);setConsole(names.length?`Installed hardware: ${names.join(", ")}. Current cost: ${money(totalCost())}.`:"The workbench is empty. Choose a motherboard platform to begin.","INSPECT")});
